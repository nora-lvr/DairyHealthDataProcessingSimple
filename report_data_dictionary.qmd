...
For a more detailed explanation of denominators see the report named "report_how_to_use_denominators"
...
fxn_graph_file_skim("denominator_by_calendar_time_period.parquet") 

calendar_options<-read_parquet('data/intermediate_files/denominator_by_calendar_time_period.parquet')
...
Calendar options include various time periods: 

`r paste0(sort(unique(calendar_options$calendar_time_period_type)), collapse = ', ')`

Various day of phase groups: 

`r paste0(sort(unique(calendar_options$day_of_phase_group)), collapse = ', ')`

This creates denominator types (combinations of lactation group and day of phase groups)

`r paste0(sort(unique(calendar_options$deno_type)), collapse = ', ')`



```{r fig.height=10, fig.width=10}
ggplot(calendar_options%>%select(deno_type, `Lactation Group`,  day_of_phase_group)%>%distinct())+
  geom_tile(aes(  x = `Lactation Group`, fill = day_of_phase_group, y = day_of_phase_group), color = 'white')+
  facet_wrap(.~`deno_type`, nrow = 2, scales = 'free_y')+
  theme_bw()+
