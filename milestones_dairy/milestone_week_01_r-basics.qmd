...
In this milestone, you'll become familiar with the `animals` data set.  You must first create this dataset by runing "STEP0_MASTER_PROCESSING.R".  

Double check that the parameters are set correctly before you run it:

## processing options -------------------------------
clean_up_old_files<-TRUE #this will delete any previously processed files as well as raw data in the event_files folder
get_EXAMPLE_herds <- 1 #(0-8) #number of Parnell Example herds you want to process.
