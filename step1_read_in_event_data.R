...
if ("SourceFarm" %in% names(events)) {
  print('Source Farm already exists')
} else {
...
mutate(technician = Technician, 
  ...
  mutate(id = ID, 
         herdid = HERDID)%>%
  mutate(animal_lact_status = case_when(
    is.na(date_archived)~'active', 
    TRUE~'archived'
  ))%>%
  mutate(dop_event = case_when(
    lact_number == 0 ~ age_event, 
    lact_number > 0 ~ dim_event, 
    TRUE~as.numeric(NA)
  ))
